...
public:
    ...
    HashString(const iter_type &begin, const iter_type &end);

    template<typename iter_type>
    void update(const iter_type &begin, const iter_type &end);

    template<typename container_type>
    HashString(const container_type &container);
...
class HashStringSlice
{
    ...
    idx_t from, to;
public:
    HashStringSlice(HashString<base, mod> *str, idx_t from, idx_t to)
...
template<uint32_t base, uint32_t mod>
template<typename iter_type>
HashString<base, mod>::HashString(const iter_type &begin,
                                  const iter_type &end) {
